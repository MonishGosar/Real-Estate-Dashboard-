...
def leases_page():
    ...
    # Add quarter and submarket filters using select boxes
    col1, col2 = st.columns(2)
    with col1:
        quarters = ["Q1", "Q2", "Q1 & Q2"]
        selected_quarter = st.selectbox("Select Quarter", quarters, index=2)
    
    with col2:
        submarkets = get_submarkets()
        selected_submarket = st.selectbox("Select Submarket", ["All"] + submarkets)

    # Convert selected quarter to database format
    db_quarters = []
    if selected_quarter in ["Q1", "Q1 & Q2"]:
        db_quarters.append(1)
    if selected_quarter in ["Q2", "Q1 & Q2"]:
        db_quarters.append(2)

    # Convert selected submarket to list format for functions
    selected_submarkets = submarkets if selected_submarket == "All" else [selected_submarket]
